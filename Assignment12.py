...
# Load the dataset
file_path = '/mnt/data/Day_12_banking_data.csv'
...
# Data Visualization
# Bar plot for total sum of Transaction_Amount per Account_Type
account_group['Transaction_Amount'].sum().plot(kind='bar', title='Total Transaction Amount per Account Type')
plt.xlabel('Account Type')
plt.ylabel('Total Transaction Amount')
plt.show()

# Pie chart for percentage of transactions per Branch
branch_group.size().plot(kind='pie', autopct='%1.1f%%', title='Percentage of Transactions per Branch')
plt.ylabel('')
plt.show()
